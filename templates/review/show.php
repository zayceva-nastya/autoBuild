...
if($_SESSION['user']['cod'] == 'user'){
    echo Html::create('TableforReviews')
    ->setControllerType('reviews')
    // ->setHeaders([1=>'text',2=>'user_name',3=>'photo'])
    ->data($table)
    ->setClass('tableReviewsUser')
    ->html();
}elseif($_SESSION['user']['cod'] == 'admin'){
    echo Html::create('TableforAnswer')
    ->setControllerType('adminanswer')
    ->setHeaders($comments)
    ->data($table)
    ->setClass('tableReviewsAdmin')
    ->html();
}
...
if ($_SESSION['user']['cod'] == 'user') {
    $form = Html::create('Form')
        ->setMethod('POST')
        ->setAction("?action=add&type=reviews")
        ->setClass('hidden')
        //    ->addClass('hidden', 'form')
        ->setId('addForm');
